...
After browsing help files via `help.start` for a bit it quickly becomes apaprent that this way of searching for help is not very efficient. Quite often we know the name of the function we need to use and all we want to do is open that particular help file. We can do this with the `help` function:
```{r, eval=FALSE}
help(topic = Trig)
```

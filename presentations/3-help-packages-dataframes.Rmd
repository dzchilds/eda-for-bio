...
## Getting data from Excel into R {.build}
...
```{r, eval = FALSE}
my.iris <- read.csv("iris_morphology.csv")
my.iris
```
...
<div id="box-text">
We want to use the `fortune` function (singular!), which resides in the `fortunes` package (plural!). Do the following, in the order given, working only in the Console (i.e. don't use your script):

1. Try to use the `fortune` function, i.e. type `fortune()` straight into the Console. Why doesn't this work?

2. Try to use `library` to load and attach the `fortunes` package. Why doesn't this work?

3. Install the `fortunes` package with `install.packages`. What happens?

4. Try again to use the `fortune` function. Why does this still not work?

5. Use `library` to load and attach the `fortunes` package (plural!). What happens?

6. Try again to use the `fortune` function. What happens?
</div>
</div>

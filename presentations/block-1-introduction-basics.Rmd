...
The `c` function 'c'ombines vectors together. We can use the assignment operator to associate a name with our vectors so that we can reuse them:
...
The lesson: __all the elements in a vector must be of the same type__. That is, we are not allowed to mix and match numbers and characters (or any other types of vector).
</div>
...
__Use the square brakets to subset vectors__. Notice that all we did was make a new vector of length one (there is nothing simpler than a vector).
...
EIf that's easy, try this as well: Imagine you want to access the last element of a vector, but you don't know how long it is (i.e. you can't use `LETTERS[26]`). How could you do it? You'll need to guess the name of a function to extract the length of a vector.
...
- Install the **fortunes** package with `install.packages`. 

- Look for the the **fortunes** package is the **Packages** tab again. 

- Try again to use the `fortune` function. Does this work or do you see an error?
</div>
...
- Use `library` to load and attach the **fortunes** package (plural). What happens at the Console?

- Try again to use the `fortune` function. Does this work now?
</div>

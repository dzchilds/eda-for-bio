...
__Using `group_by` to group by more than one variable__
<div id="box-text">
Let's repeat the earlier `group_by` and `summarise` exercise, using the pipe. Remember, we used the `group_by` and `summarise` functions with the `storms` dataset to calculate the mean and maximum wind speed (in the `wind` variable) associated with each combination of month and year.
</div>
</div>
